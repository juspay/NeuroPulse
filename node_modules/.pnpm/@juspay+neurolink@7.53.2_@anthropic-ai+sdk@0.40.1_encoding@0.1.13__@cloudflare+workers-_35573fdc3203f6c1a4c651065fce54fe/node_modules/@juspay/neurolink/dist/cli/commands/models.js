...
export class ModelsCommandFactory {
    ...
    static async executeSearch(argv) {
        try {
            ...
            logger.error(chalk.red(`❌ Search command failed: ${error.message}`));
            process.exit(1);
        }
    }
    /**
    ...
    static async executeBest(argv) {
        try {
            ...
            if (argv.format === "json") {
                ...
                logger.always(`${chalk.green("✅")} ${chalk.cyan(model.id)} (${model.name})`);
                logger.always(`   Provider: ${model.provider} | Category: ${model.category}`);
            ...
            logger.error(chalk.red(`❌ Best command failed: ${error.message}`));
            process.exit(1);
        }
    }
    /**
    ...
    static async executeResolve(argv) {
        try {
            ...
            logger.error(chalk.red(`❌ Resolve command failed: ${error.message}`));
            process.exit(1);
        }
    }
    /**
    ...
    static async executeCompare(argv) {
        try {
            ...
            logger.error(chalk.red(`❌ Compare command failed: ${error.message}`));
            process.exit(1);
        }
    }
    /**
    ...
    static async executeStats(argv) {
        try {
            ...
            logger.error(chalk.red(`❌ Stats command failed: ${error.message}`));
            process.exit(1);
        }
    }
}
