...
export class MCPCommandFactory {
    ...
    static async executeInstall(argv) {
        try {
            ...
            if (spinner) {
                spinner.succeed(chalk.green(`✅ Successfully installed ${serverName} MCP server`));
            }
            ...
            try {
                ...
                if (installedServer?.status === "connected") {
                    ...
                    if (installedServer.tools?.length) {
                        logger.always(`🛠️  Available tools: ${installedServer.tools.length}`);
                    }
                }
                else {
    ...
    static async executeAdd(argv) {
        try {
            ...
            if (spinner) {
                spinner.succeed(chalk.green(`✅ Successfully added ${name} MCP server`));
            }
        ...
        try {
            ...
            for (const server of serversToTest) {
                ...
                if (server.status === "connected") {
                    ...
                    logger.always(chalk.yellow("  💡 Try: neurolink mcp remove && neurolink mcp install"));
                }
                logger.always();
            }
            // Summary
            ...
            if (connected === total) {
                logger.always(chalk.green(`🎉 All ${total} servers connected successfully`));
            }
            else {
    ...
    static async executeRemove(argv) {
        try {
            ...
            if (!argv.force) {
                logger.always(chalk.yellow(`⚠️  This will remove the MCP server: ${serverName}`));
            ...
            try {
                ...
                if (spinner) {
                    spinner.succeed(chalk.green(`✅ Server ${serverName} removed successfully`));
                }
                logger.always(chalk.green(`🗑️  Successfully removed MCP server: ${serverName}`));
    ...
    static async executeDiscover(argv) {
        try {
            ...
            if (discovered.length === 0) {
                ...
                logger.always(chalk.blue("💡 Try installing popular servers: neurolink mcp install filesystem"));
                return;
            }
